...
my $num_user_to_contact = $q->param('chatCombo');
...
$session->param("chat_index", $num_user_to_contact);
...
if( $num_el != 0 )
{
    ...
    if( $email_dest and $email_src )
    {
        ...
        my $message="yo";
	my @uids = getUidsForChat( $messageID, $sid );
        my $tmp=$uids[$num_user_to_contact];

        $session->flush();
        $session->close();
        generateChatForm( $sid, $messageID, $MyID, $message, $uids[$num_user_to_contact] , $num_user_to_contact);
	#print "\$num_user_to_contact = $num_user_to_contact";
    }
    else
    {
