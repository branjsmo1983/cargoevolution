...
sub getTripInformationFromMID
{
   ...
   # $message_details="$message_details$tmp1";
    ...
    if( $DL1 ne $DL2 )
    {
      $tmp1=" Data di carico A:     $DL1 \n";
      $message_details="$message_details$tmp1\n";
      $tmp1= "Data di carico B:  $DL2 \n";
      $message_details="$message_details$tmp1\n";
    }else{
      $tmp1=" Data di carico :     $DL1\n";
    $message_details="$message_details$tmp1\n";
    }
    ...
    $tmp1 = " Luogo di ritiro:    $PUP_province, $PUP_region\n";
    $message_details="$message_details$tmp1\n";
    $tmp1 = " Luogo di consegna:  $DEL_province, $DEL_region\n";
    $message_details="$message_details$tmp1\n";
    $tmp1 = " Tipologia di mezzo: $vtype_string \n";
    $message_details="$message_details$tmp1\n";
    # print vehicle details as well
    my $length = $vehicle_details[2];
    $tmp1 = " Metri di pianale:   $length \n";
    $message_details="$message_details$tmp1\n";
    my $weight = $vehicle_details[3];
    $tmp1 = " Peso (tonnellate):  $weight \n";
    $message_details="$message_details$tmp1\n";
